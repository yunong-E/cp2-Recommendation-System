...
## 23.04.10 (월)
- 팀원분께서 저의 (EDA와 인사이트 도출이 담긴)코드 파일과 팀원분의 (모델링이 담긴)코드 파일을 통합하는 작업을 진행중입니다. 이 작업과 더불어 발표 자료를 제작합니다. [23.04.11 예상치 못한 변수의 등장과 시간이 부족한 상황 등을 고려하여 병합작업은 진행하지 않게 되었습니다.]
...
## 23.04.13 (목)
- 팀원분과 함께 발표자료에 반영될 `자체평가의견`에 대해 공유하는 시간을 가졌습니다.
- 19시경, 팀원으로부터 발표 영상 촬영을 위한 pptx와 대본 파일을 전달받음.

<br/>

## 23.04.14 (금)
- 오전중, 리허설 진행 및 대본 일부 수정.
- 오후에 영상촬영을 완료하여 팀원분에게 전달.
- 프로젝트(cp2) 종료.
- 코딩테스트를 위한 SQL언어 학습 및 마케팅을 위한 분석(코호트 분석, 리텐션 등)학습 시작. (~ 23.04.30)
